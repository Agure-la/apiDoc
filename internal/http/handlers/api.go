...
func (h *APIHandler) ListAPIs(w http.ResponseWriter, r *http.Request) {
	...
	http.WriteJSONResponse(w, http.StatusOK, apis)
}

func (h *APIHandler) GetAPI(w http.ResponseWriter, r *http.Request) {
	apiName := strings.TrimPrefix(r.URL.Path, "/apis/")
	if apiName == "" {
		http.WriteError(w, http.StatusBadRequest, "API name is required")
		return
	}
	...
	http.WriteJSONResponse(w, http.StatusOK, api)
}

func (h *APIHandler) ListAPIVersions(w http.ResponseWriter, r *http.Request) {
	pathParts := strings.Split(strings.Trim(r.URL.Path, "/"), "/")
	if len(pathParts) < 3 {
		http.WriteError(w, http.StatusBadRequest, "Invalid URL format")
		return
	}
	...
	http.WriteJSONResponse(w, http.StatusOK, versions)
}
...
func (h *APIHandler) GetAPIVersion(w http.ResponseWriter, r *http.Request) {
	...
	http.WriteJSONResponse(w, http.StatusOK, apiVersion)
}
